...
def Home():
	...
	print("\t\t\t\t\t\t WELCOME TO HOTEL PHOENIX\n")
	print("\t\t\t 1 Booking\n")
	print("\t\t\t 2 Rooms Info\n")
...
def date(c):
	...
	if c[0] >= 2022 and c[0] <= 2050:
		...
		if c[1] != 0 and c[1] <= 12:
			...
			if c[1] == 2 and c[2] != 0 and c[2] <= 31:
				
				if c[0]%4 == 0 and c[2] <= 29:
					pass
			...
			# to 7th month
			elif c[1] <= 7 and c[1]%2 != 0 and c[2] <= 31:
				pass
			
			# if month is even & less than equal to 7th
			# month and not 2nd month
			elif c[1] <= 7 and c[1]%2 == 0 and c[2] <= 30 and c[1] != 2:
				pass
			
			# if month is even & greater than equal
			# to 8th month
			elif c[1] >= 8 and c[1]%2 == 0 and c[2] <= 31:
				pass
			
			# if month is odd & greater than equal
			# to 8th month
			elif c[1]>=8 and c[1]%2!=0 and c[2]<=30:
				pass
...
def Booking():
		...
		cii=str(input("Check-In(yyyy/mm/dd): "))
		checkin.append(cii)
		cii=cii.split('/')
		ci=cii
		ci[0]=int(ci[0])
...
def Record():
	...
	if phno!=[]:
		...
		for n in range(0,i):
			print("|",ename[n],"\t |",phno[n],"\t|",addr[n],"\t|",checkin[n],"\t|",checkout[n],"\t|",room[n],"\t|",price[n])
